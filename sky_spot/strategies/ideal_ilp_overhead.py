...
class IdealILPStrategy(strategy.Strategy):
    ...
    def register_env(self, env):
        ...
        # Prepare constants
        on_demand_per_gap_cost = COSTS[ClusterType.ON_DEMAND] * env.gap_seconds / 3600
        spot_per_gap_cost = COSTS[ClusterType.SPOT] * env.gap_seconds / 3600
        
        # Define the variables
        a = pulp.LpVariable.dicts("a", range(self.total_gaps), cat=pulp.LpBinary)
        b = pulp.LpVariable.dicts("b", range(self.total_gaps), cat=pulp.LpBinary)
        x = pulp.LpVariable.dicts("x", range(self.total_gaps - 1), cat=pulp.LpBinary) # intermediate variable
        y = pulp.LpVariable.dicts("y", range(self.total_gaps - 1), cat=pulp.LpBinary) # intermediate variable

        # Define the objective function
        objective = pulp.lpSum(a[i] * spot_per_gap_cost + b[i] * on_demand_per_gap_cost for i in range(self.total_gaps))
        prob += objective, "Total Cost"

        # Define the constraints
        for i in range(self.total_gaps):
            prob += a[i] + b[i] <= 1,               f"Only one of the a[{i}], b[{i}] variables can be 1"
            prob += a[i] <= (1 - self.trace[i]),    f"a[{i}] is 1 only if there is spot"
            if i < self.total_gaps - 1:
                prob += x[i] <= a[i + 1],           f"x[{i}] can be 1 only if a[{i+1}] is 1"
                prob += x[i] <= 1 - a[i],           f"x[{i}] can be 1 only if a[{i}] is 0"
                prob += x[i] >= a[i + 1] - a[i],    f"x[{i}] can be 1 only if a[{i+1}] is 1 and a[{i}] is 0"
        ...
        if verbose:
            print(f"ILP Status: {pulp.LpStatus[status]}\tObjective: {objective}\t"
